...
class ConditionalNAFNet(nn.Module):
    ...
    def __init__(self, img_channel=3, cond_channels=3, width=16, middle_blk_num=1,
        ...
        self.intro = nn.Conv2d(in_channels=img_channel + cond_channels, out_channels=width,
                               kernel_size=3, padding=1, stride=1, groups=1, bias=True)
        self.ending = nn.Conv2d(in_channels=width, out_channels=img_channel, kernel_size=3, padding=1, stride=1,
                                groups=1, bias=True)

        # Extra condition channels (e.g. Retinex R/I) set externally before forward
    ...
    def forward(self, inp, cond, time):
        # Inject extra condition channels (e.g. Retinex R/I) if set externally
        if self.extra_cond is not None:
            b = inp.shape[0]
            extra = self.extra_cond[:b].to(inp.device)
