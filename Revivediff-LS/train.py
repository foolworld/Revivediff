...
def main():
    ...
    if rank <= 0:  # normal training (rank -1) OR distributed training (rank 0-7)
        if resume_state is None:
            ...
            make_link(os.path.join(opt["path"]["experiments_root"], ".."), "./log")
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            # SDE uses only first 3 channels (original LQ) as mean μ;
            # extra channels (R/I from Retinex) are injected inside the network
            mu_LQ = LQ[:, :3, :, :] if LQ.shape[1] > 3 else LQ

            # Adaptive sigma: compute scale and set on SDE BEFORE generating states
            # This ensures sigma_bar used in generate_random_states matches optimize_parameters
            if hasattr(model, 'adaptive_sigma') and model.adaptive_sigma:
                sigma_scale = model.compute_sigma_scale(mu_LQ.to(model.device))
                sde.set_sigma_scale(sigma_scale)

            timesteps, states = sde.generate_random_states(x0=GT, mu=mu_LQ)
            ...
            if current_step % opt["train"]["val_freq"] == 0 and rank <= 0:
                ...
                for _, val_data in enumerate(val_loader):
                    ...
                    # SDE noise only on first 3 channels (original LQ)
                    mu_LQ = LQ[:, :3, :, :] if LQ.shape[1] > 3 else LQ

                    # Adaptive sigma for validation
                    if hasattr(model, 'adaptive_sigma') and model.adaptive_sigma:
                        with torch.no_grad():
                            val_sigma_scale = model.compute_sigma_scale(mu_LQ.to(model.device))
                        sde.set_sigma_scale(val_sigma_scale)

                    noisy_state = sde.noise_state(mu_LQ)
                    ...
                    sr_img_y = bgr2ycbcr(output, only_y=True)
