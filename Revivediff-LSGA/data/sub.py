...
from util import ProgressBar  # noqa: E402
...
def main():
    ...
    parser.add_argument("--input", help="Input folder for single mode.")
    parser.add_argument("--save", help="Save folder for single mode.")
    parser.add_argument("--gt", help="GT folder for pair mode.")
    parser.add_argument("--lq", help="LQ folder for pair mode.")
    parser.add_argument("--save-gt", help="Cropped GT save folder for pair mode.")
    parser.add_argument("--save-lq", help="Cropped LQ save folder for pair mode.")
    parser.add_argument("--crop-size", type=int, default=128)
    parser.add_argument("--step", type=int, default=128)
    parser.add_argument("--threshold-size", type=int, default=48)
    parser.add_argument("--scale-ratio", type=int, default=1)
    parser.add_argument("--threads", type=int, default=20)
    args = parser.parse_args()

    mode = args.mode  # single (one input folder) | pair (extract corresponding GT and LR pairs)
    opt = {}
    opt['n_thread'] = args.threads
    opt['compression_level'] = 3  # 3 is the default value in cv2
    # CV_IMWRITE_PNG_COMPRESSION from 0 to 9. A higher value means a smaller size and longer
    # compression time. If read raw images during training, use 0 for faster IO speed.
    if mode == 'single':
        if not args.input or not args.save:
            raise ValueError("--input and --save are required in single mode.")
        opt['input_folder'] = args.input
        opt['save_folder'] = args.save
        opt['crop_sz'] = args.crop_size
        opt['step'] = args.step
        opt['thres_sz'] = args.threshold_size
        extract_signle(opt)
    elif mode == 'pair':
        if not all([args.gt, args.lq, args.save_gt, args.save_lq]):
            raise ValueError("--gt, --lq, --save-gt and --save-lq are required in pair mode.")
        GT_folder = args.gt
        LR_folder = args.lq
        save_GT_folder = args.save_gt
        save_LR_folder = args.save_lq
        scale_ratio = args.scale_ratio
        crop_sz = args.crop_size
        step = args.step
        thres_sz = args.threshold_size
        ########################################################################
