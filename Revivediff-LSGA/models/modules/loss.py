...
def charbonnier_loss(predict, target, eps=1e-6):
    """Charbonnier loss, more robust to outliers than L1."""
    return torch.sqrt((predict - target) ** 2 + eps ** 2).mean()


class SobelEdgeLoss(nn.Module):
    """Differentiable Sobel edge loss, replaces non-differentiable Canny."""
    def __init__(self):
        super().__init__()
...
def focal_frequency_loss(predict, target):
    """Frequency-domain loss weighted toward high-frequency components."""
...
def histogram_loss(enhanced_tensor, hq_tensor, bins=128):
    """Differentiable histogram loss using soft histogram."""
...
def pearson_loss_fn(predict, target, eps=1e-6):
    """Numerically stable Pearson correlation coefficient loss."""
    losses = []
...
class MatchingLoss(nn.Module):
    """Combined loss supporting Charbonnier, Sobel, Histogram, Focal Frequency and Pearson."""
