...
class ConditionalNAFNet(nn.Module):
    ...
    def __init__(self, img_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[], upscale=1, T=300):
        super().__init__()
    ...
    def forward(self, inp, cond, time):
        ...
        # Time-dependent condition modulation (inspired by UniLDiff DAFF)
        # Early timesteps (high noise) → more LQ guidance (alpha → 1.0)
        # Late timesteps (low noise) → less LQ guidance (alpha → 0.2),
        # letting model focus on detail refinement
        if isinstance(time, torch.Tensor):
            t_norm = time.float().mean() / self.T
        else:
            t_norm = time / self.T
        alpha = 0.2 + t_norm * 0.8   # range: [~0.2, 1.0]

        x = inp - alpha * cond[:, :self.img_channel, :, :]
        x = torch.cat([x, alpha * cond[:, :self.img_channel, :, :]], dim=1)
